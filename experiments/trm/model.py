...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    ...
    # --- FIX: Accept 'device' argument to create tensors on GPU ---
    ...
    def reset_carry(self, reset_flag: torch.Tensor, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry):
        # Broadcast H_init/L_init to match batch/seq dims
        # reset_flag: [B] -> [B, 1, 1]
        mask = reset_flag.view(-1, 1, 1)
        
        # self.H_init is [D]. We need [B, L, D] for torch.where if carry is [B, L, D]
        # Or let broadcasting handle [D] vs [B, L, D].
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry, batch: Dict[str, torch.Tensor]) -> Tuple[TinyRecursiveReasoningModel_ACTV1InnerCarry, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        q_logits = self.q_head(z_H.mean(dim=1)) 
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    ...
    # --- FIX: Detect device from input batch and pass it down ---
    def initial_carry(self, batch: Dict[str, torch.Tensor]):
        batch_size = batch["inputs"].shape[0]
        device = batch["inputs"].device # Get correct device (e.g. cuda:0)
        ...
        return TinyRecursiveReasoningModel_ACTV1Carry(
            ...
            # Use empty_like to preserve device of the source tensor
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1Carry, batch: Dict[str, torch.Tensor]):
        ...
        if self.training:
            ...
            halted = halted | (should_halt ^ rand_halt) # Explore
        else:
            # Inference greedy
            if self.config.no_ACT_continue:
